//Whip's Tank Skid Steering System v19 - 10/11/18

const string ignoreGyroNameTag = "Ignore";
    ...
    {    
        ...
        else //S
        {
            ...
            else
            {
                if (inputVec.X < 0) //A
                {
                    TurnLeft(leftWheels, rightWheels, gyros, inputVec, true);
                }
                else if (inputVec.X > 0) //D
                {
                    TurnRight(leftWheels, rightWheels, gyros, inputVec, true);
                }
                else
                {
                    NoTurn(leftWheels, rightWheels, gyros);
...
bool GrabBlocks()
{
    ...
    if (useGyros)
    {
        GridTerminalSystem.GetBlocksOfType(gyros, block => block.CubeGrid == Me.CubeGrid && !block.CustomName.Contains(ignoreGyroNameTag));
        if (gyros.Count == 0)
        {
...
void NoTurn(List<IMyMotorSuspension> leftWheels, List<IMyMotorSuspension> rightWheels, List<IMyGyro> gyros)
{
    ...
    SetGyroPower(gyros, driveGyroPower);
}

void TurnRight(List<IMyMotorSuspension> leftWheels, List<IMyMotorSuspension> rightWheels,  List<IMyGyro> gyros, Vector3D inputVec, bool invertSteer = false)
{
    InvertWheelPropulsion(leftWheels, true);
    InvertWheelPropulsion(rightWheels, false);
    InvertSteering(leftWheels, invertSteer ? false : true);
    InvertSteering(rightWheels, invertSteer ? true : false);
    SetFriction(leftWheels, turnFriction);
    SetFriction(rightWheels, turnFriction);
    SetGyroPower(gyros, turnGyroPower);
    if (inputVec.Z <= 0)
...
void TurnLeft(List<IMyMotorSuspension> leftWheels, List<IMyMotorSuspension> rightWheels,  List<IMyGyro> gyros, Vector3D inputVec, bool invertSteer = false)
{
    InvertWheelPropulsion(leftWheels, false);
    InvertWheelPropulsion(rightWheels, true);
    InvertSteering(leftWheels, invertSteer ? true : false);
    InvertSteering(rightWheels, invertSteer ? false : true);
    SetFriction(leftWheels, turnFriction);
    SetFriction(rightWheels, turnFriction);
    SetGyroPower(gyros, turnGyroPower);
    if (inputVec.Z <= 0)
...
void GetWheelSide(IMyShipController reference, List<IMyMotorSuspension> wheels, out List<IMyMotorSuspension> leftWheels, out List<IMyMotorSuspension> rightWheels)
{
    leftWheels = new List<IMyMotorSuspension>();
    rightWheels = new List<IMyMotorSuspension>();
    var centerOfMass = reference.CenterOfMass;
    var leftVector = reference.WorldMatrix.Left;

    foreach (IMyMotorSuspension thisWheel in wheels)
    {
        var displacement = thisWheel.GetPosition() - centerOfMass;
        if (displacement.Dot(leftVector) >= 0) //left wheel
            leftWheels.Add(thisWheel);
        else if (displacement.Dot(leftVector) < 0)
