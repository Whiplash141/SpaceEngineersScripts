...
const int dispenseTime = 15; //minutes
...
/*
/ //// / Whip's Cargo Refill TLB - version 3 - 2/26/18 / //// /
...
const int runsPerBeaconUpdate = 3;
int currentRun = 0;
    ...
    if ((updateSource & UpdateType.Update100) != 0)
    {
        ...
        switch (currentStatus)
        {
            case Status.Countdown:
                ...
                {
                    if (currentRun >= runsPerBeaconUpdate)
                    {
                        foreach (var block in beacons)
                        {
        ...
        //Check state after
        if (currentStatus == Status.Idle) //this will only run once
        {
            foreach (var block in beacons)
            {
                block.CustomName = $"Resources Ready";
