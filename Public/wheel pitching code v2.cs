...
//Whip's Wheel Pitching Code v2 - 7/28/18
//This is not churro's code
const double mouseSensitivity = 0.0001;
const string wheelIgnoreName = "Ignore";
const double maxActiveAltitude = 10; //When above this altitude, wheels will go to max extension
...
void Main(string arg, UpdateType updateSource)
{
    ...
    GridTerminalSystem.GetBlocksOfType(wheels, x => !x.CustomName.Contains(wheelIgnoreName));
    if (wheels.Count == 0)
        return;
    ...
    var maxHeight = 0f; //wheels[0].GetMaximum<float>("Height");
