...
/*
/ //// / Whip's Cargo Refill TLB - version 1 - 2/25/18 / //// /
...
void Main(string arg, UpdateType updateSource)
{
        ...
        switch (currentStatus)
        {
            case Status.Countdown:
                ...
                else
                {
                    foreach (var block in beacons)
                    {
                        block.CustomName = $"Resources Incoming: {GetComponentString()} (T - {dispenseCountdown.Minutes}:{dispenseCountdown.Seconds:00})";
                    }
                }
            ...
            case Status.Cooldown:
                ...
                foreach (var block in beacons)
                {
                    block.CustomName = $"Resource Cooldown (T - {cooldown.Minutes}:{cooldown.Seconds:00})";
                }
    ...
    Echo(RunningSymbol());
    Echo(currentStatus.ToString());
    Echo($"arg: {arg}");
    Echo($"dispense - {dispenseCountdown.Minutes}:{dispenseCountdown.Seconds:00}");
    Echo($"cooldown - {cooldown.Minutes}:{cooldown.Seconds:00}");
